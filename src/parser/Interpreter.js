...
const setObservable = (prop, data, directive) => {
  ...
  if (directive) {
    if (data && data.hasOwnProperty(prop) && data.__observable__) {
      let subscribed = data.__observable__.subscribe(directive, prop)
      ...
      if (data) {
        if (data.constructor.name === 'Mask') {
          ...
          subscribed = data.__observable__.subscribe(directive, prop)
          if (subscribed && directive.observables.indexOf(data.__observable__) === -1) {
            directive.observables.push(data.__observable__)
          }
          data.forEach(entry => {
  ...
  tree
    .map((node, i) => {
      ...
      if (type === 'tenary') {
        ...
        let prop = path.shift()
        let output = prop === '$route' ? env.this[prop] : env.this.data[prop]
        // This is shit
